...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
	...
	if(dist < MIN_SAFE_DISTANCE && width < MIN_SAFE_WIDTH
                && dist > START_DIST && width > START_WIDTH){
            //&& width < MIN_SAFE_WIDTH){
           ROS_INFO("Ping index = %d, Ping_distance = %f", ping_index_, laser_scan.ranges[i]);
           ROS_INFO("Index angle = %f", index_angle);
    ...
    if(!laser_alarm_ && srvcall && !ESTOP_){
