...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
    ...
    for(int i = 0; ((temp_alarm == false) && (i < 640)) ; i += 1){
	...
	float distance = 0.0;	
	if(laser_scan.ranges[i] == NAN){
		distance = 4.0;
	}	
	else{
		distance = laser_scan.ranges[i];
	}
	ROS_INFO("Distance = %f, i = %d, size = %d", distance, i, laser_scan.ranges.size()); //Width = %f, Index angle = %f, Laser ranges = %f", dist, width, index_angle, laser_scan.ranges[i]);
	if(distance < MIN_SAFE_DISTANCE){ //&& width < MIN_SAFE_WIDTH
   	   temp_alarm = true;
   	   temp_index = i;
   	}
    }
